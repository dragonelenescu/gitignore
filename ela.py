...
with SB(uc=True, test=True) as sb:
    ...
    if True:
        ...
        if sb.is_element_visible('#injected-channel-player'):
            sgaxxz = sb.get_new_driver(undetectable=True)
            sgaxxz.uc_open_with_reconnect(url, 5)
            sgaxxz.uc_gui_click_captcha()
            sgaxxz.uc_gui_handle_captcha()
            sb.sleep(10)
            if sgaxxz.is_element_present('button:contains("Accept")'):
                sgaxxz.uc_click('button:contains("Accept")', reconnect_time=4)
